...
import { generateQuotePDF } from '../services/printing';
...
export const SalesArea: React.FC<SalesAreaProps> = ({ isOpen, onClose, onEditItems, currentUser }) => {
  ...
  // Changed types to allow string (for empty inputs)
  const [localDiscounts, setLocalDiscounts] = useState<Record<string, string | number>>({});
  const [localShippingCosts, setLocalShippingCosts] = useState<Record<string, string | number>>({});
  ...
  const handleDiscountBlur = (order: Order) => {
    ...
    if (val !== undefined && val !== '') {
        ...
        else updateOrderTotals(order, { discount: 0 });
    }
  };
  ...
  const handleShippingCostBlur = (order: Order) => {
    ...
    if (val !== undefined && val !== '') {
        ...
        else updateOrderTotals(order, { shippingCost: 0 });
    }
  };
  ...
  const handleDownloadQuote = (order: Order) => {
      generateQuotePDF(order, settings, logo);
  };
...
${finalMessage}`;
    ...
    const phoneNumber = '5562992973853'; 
                ...
                <div key={order.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                      ...
                      {!isEditingAddr ? (
                          ...
                          <div className="space-y-3 bg-zinc-900 p-4 rounded-xl border border-zinc-700 shadow-xl animate-fade-in relative overflow-hidden">
                              ...
                              <div className="grid grid-cols-3 gap-2">
                                  <div className="col-span-1"><input placeholder="CEP" className="w-full bg-zinc-800 border border-zinc-600 p-2 text-xs rounded text-white" value={addressForm.cep} onChange={e => setAddressForm({...addressForm, cep: e.target.value})} onBlur={handleCepBlur}/></div>
                                  <div className="col-span-2"><input placeholder="Cidade" className="w-full bg-zinc-950 border border-zinc-800 p-2 text-xs rounded text-gray-500 font-medium" value={addressForm.city} readOnly/></div>
                              </div>
                              <div><input placeholder="Rua" className="w-full bg-zinc-800 border border-zinc-600 p-2 text-xs rounded text-white" value={addressForm.street} onChange={e => setAddressForm({...addressForm, street: e.target.value})}/></div>
                              <div className="grid grid-cols-3 gap-2">
                                  <div className="col-span-1"><input placeholder="Nº" className="w-full bg-zinc-800 border border-zinc-600 p-2 text-xs rounded text-white" value={addressForm.number} onChange={e => setAddressForm({...addressForm, number: e.target.value})}/></div>
                                  <div className="col-span-2"><input placeholder="Bairro" className="w-full bg-zinc-800 border border-zinc-600 p-2 text-xs rounded text-white" value={addressForm.district} onChange={e => setAddressForm({...addressForm, district: e.target.value})}/></div>
                              </div>
                              <div><input placeholder="Complemento" className="w-full bg-zinc-800 border border-zinc-600 p-2 text-xs rounded text-white" value={addressForm.complement} onChange={e => setAddressForm({...addressForm, complement: e.target.value})}/></div>
                              <div className="flex justify-end gap-2 pt-3 border-t border-zinc-800">
                                  <button onClick={cancelEditingAddress} className="text-xs text-gray-400 hover:text-white px-2">Cancelar</button>
                                  <button onClick={() => saveAddress(order)} className="text-xs bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded font-bold shadow-lg">Salvar Endereço</button>
                              </div>
                          </div>
                      )}
                  </div>
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex flex-wrap justify-end gap-3 mb-4">
                       {/* Replaced Old Button with Download PDF */}
                       <button 
                         onClick={() => handleDownloadQuote(order)}
                         className="w-full sm:w-auto text-xs font-bold text-white bg-zinc-800 hover:bg-zinc-700 flex items-center justify-center gap-2 px-4 py-2 rounded-lg shadow-md transition-colors"
                       ...
                       {(order.status === 'orcamento' && !isLockedForEditing) && (
                           <button 
                             ...
                             className="text-xs font-bold text-blue-600 hover:text-blue-800 hover:bg-blue-50 px-3 py-1.5 rounded-full flex items-center gap-1 transition-colors border border-blue-200"
                           >
                             Editar Itens
                           </button>
                       )}
                    ...
                    <div className="bg-gray-50 p-3 rounded-lg space-y-3">
                      ...
                      {/* Discount Input - Shows empty if 0 */}
                      <div className="flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-1 w-24">
                          ...
                          <input 
                            ...
                            placeholder="0,00"
                            value={localDiscounts[order.id] !== undefined ? localDiscounts[order.id] : (order.discount === 0 ? '' : order.discount)}
                      ...
                      {/* Shipping Controls - Shows empty if 0 */}
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <div className="flex items-center gap-1 w-24">
                              ...
                              <input 
                                ...
                                placeholder="0,00"
                                value={localShippingCosts[order.id] !== undefined ? localShippingCosts[order.id] : (order.shippingCost === 0 ? '' : order.shippingCost)}
